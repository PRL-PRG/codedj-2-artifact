...
if ! [ -f smells/stars/LargeClass.csv ]; then
  curl -o smells.tgz "https://github.com/PRL-PRG/codedj-2-artifact/releases/download/v0.0/smells.tgz" -L
  tar -zxvf smells.tgz
  rm smells.tgz
fi
```
...
`reproductions/scent_dl/dl_projects_cloner` over the datastore of deep learning 
projects.

2. About the folder `smells/smells_dl`:
...
- The tool tool to get the smells is located at `reproductions/scent_dl/tools/pysmell`. 
The instructions to run this tool are located at `reproductions`/scent_dl/tools/README.md`.

3. To run the tool to detect smells you should first clone repositories. 
The instructions to clone repositories are in `reproductions/scent_dl/tools/README.md` 
as well.
...
```{r}
#loads a dataframe that contains data of smells for top 106 starred projects
...
```{r}
get_indices_strat = function (num_samples=10, seed=111,filename="", target_population=NULL) {
    set.seed(seed)
...
The djanco recipe for this filter is 
`artifacts/scent_dl/traditional-queries/src/lib.rs::custom_query`
```{r}
dev_projects_population = python %>%
                          filter(max_hindex1 >= 3) %>%
                          filter(age >= 364) %>%
                          filter(users >= 3) %>%
                          filter(locs >= 716) %>%
                          filter(snapshots >= 20) %>%
...
indices = get_indices_strat(num_samples=5, seed=43, filename="dev_project_indices.csv", target_population=dev_projects_population)
```
...
plot_all_samples_single_graph = function(num_samples_=10,seed_=111, filenames=c(), target_populations=c()){
    ...
    for(filename in filenames){
      ...
      # we only use these 3 samples
      for (i in 1:3) {
          indices = get_stratified_sample(n=i,num_samples=num_samples_,seed=seed_,target_population = dev_projects_population)
...
```{r}
smell_frequencies = plot_all_samples_single_graph(num_samples_=5, filenames=c('dev_projects_smells'), seed_=43, target_populations=c())
smell_frequencies
ggsave("figs/scent-dl/smell_frequencies.pdf", plot = smell_frequencies, height=3, width=6 )
```
*Original claims:*
- Small DL projects have much more smells than developed projects.

*Observations:*
- if we say top-starred are developed then we confirm the findings of the papers.
- If we take subset of developed projects then we do not confirm. There is not statistical diff. in columns 3 and 5 
- The sample size is likely too small, because we see great variance in developed projects samples (isuch as in column 4).


*Note:*
Maybe we wanna show only columns 3-4 or 4-5.

-------------

*Original claims:*
- Medium DL projects have not statistical difference.

*Observations:*
- We confirm on both top-starred and dev. samples.


--------------

*Original claims:*
- Large DL projects have fewer smells than developed projects.

*Observations:*
- We dont confirm for either top-starred and sample of developed projects

*Note:*
- We should elaborate on how to confirm the results of top-starred projects.
