...
if ! [ -f local/java-sample ]; then
  curl -o local.tgz "https://github.com/PRL-PRG/codedj-2-artifact/releases/download/v0.0/what-constitutes-software-local.tgz" -L
  tar -zxvf local.tgz
  rm local.tgz     
fi
```
...
import_codedj_results_updated = function(path) {
    result = list()
...
sw_java = import_codedj_results_updated("reproductions/what-constitutes-software/results/java")
sw_python = import_codedj_results_updated("reproductions/what-constitutes-software/results/python")
sw_js = import_codedj_results_updated("reproductions/what-constitutes-software/results/js")
```
