...
The categorization was done manually, by greedily checking the artifact types until all but tiny percentage were classified ([details](https://github.com/HelgeCPH/msr2020_what_constitutes_software/blob/ffffc004547ecadc1524f95c726accadae03b706/src/interpretation.py#L655)). 
    ...
    ./java -o /mnt/data/codedj-2-artifact/reproductions/what-constitutes-software/results2/java -d /mnt/data/codedj-2-artifact/dataset/java -c /mnt/data/codedj-2-artifact/djanco-cache/java  
    ./python -o /mnt/data/codedj-2-artifact/reproductions/what-constitutes-software/results2/python -d /mnt/data/codedj-2-artifact/dataset/python -c /mnt/data/codedj-2-artifact/djanco-cache/python  
    ./js -o /mnt/data/codedj-2-artifact/reproductions/what-constitutes-software/results2/js -d /mnt/data/codedj-2-artifact/dataset/js -c /mnt/data/codedj-2-artifact/djanco-cache/js  
...
We use djanco to obtain the samples for stars, random sample and developed projects as devised by the paper:

```{r}

import_codedj_results = function(path) {
    result = list()
    result$s0 = read_csv(paste0(path, "/sample_all.csv"))
    result$s1 = read_csv(paste0(path, "/sample_stars.csv"))
    result$s2 = read_csv(paste0(path, "/sample_developed.csv"))
    result
}

sw_java = import_codedj_results("dataset/reproductions/what-constitutes-software/results/java")
sw_python = import_codedj_results("dataset/reproductions/what-constitutes-software/results/python")
sw_js = import_codedj_results("dataset/reproductions/what-constitutes-software/results/js")
```
...
d$dataset = factor(d$dataset, levels = rev(c("S0", "S1", "S2")))
...
g = ggplot(d, aes(fill = dataset, x = kind, y = pct, label = paste0(round(pct,2) * 100, "%"))) +
  ...
  scale_fill_manual(name = NULL, values = rev(c("gray", "#ff0000", "#0000ff"))) +
  coord_flip() +
  theme_minimal() +
...
g
```
`C1`: Software is diverse, only 4% of repositories do not contain code, data and documentation. 
`C2`: Documentation is an integral contituent of software, only 2% of repositories do not contain documentation. 

Therefore `C1` is the percentage of repositories that do not contain at least one file in all three categories, `C2` is percentage of projects that do not contain documentation files at all. 

`S0` (gray): is random sample from the entire dataset
`S1` (red): is top 1k projects by # of stars
`S2` (blue): is 1k random sample from the developed projects
